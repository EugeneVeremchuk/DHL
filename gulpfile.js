...
function favicon() {
	return src(path.source.favicon)
		.pipe(plumber())
		.pipe(
			rename({
				extname: ".ico"
...
function fontstyle() {
	let file_content = fs.readFileSync(source_folder + '/scss/settings/fonts.scss');
	if (file_content == '') {
			...
			if (items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (c_fontname != fontname) {
						fs.appendFile(source_folder + '/scss/settings/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
					}
					c_fontname = fontname;
				}
			}
